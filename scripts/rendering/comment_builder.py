...
def cve_lines(pr):
    ...
    if fixes:
        ...
        out.append('**CVE context:** ' + ', '.join(f'`{c}`' for c in list(dict.fromkeys(pr.get('cves')))[:6]) + ' (from PR/advisory metadata; reachability is not proven).')
    else:
...
def main():
    try:
        ...
        print(f"WARNING: failed to parse PR_FIELDS JSON, defaulting to empty: {exc}", file=sys.stderr)
        fields = {}
    try:
        with open(os.environ.get('RESULTS_FILE', '/tmp/build-results.json')) as fh:
            data = json.load(fh)
    except Exception as exc:
        print(f"WARNING: failed to read/parse RESULTS_FILE, defaulting to empty: {exc}", file=sys.stderr)
        data = {'prs': {}}
